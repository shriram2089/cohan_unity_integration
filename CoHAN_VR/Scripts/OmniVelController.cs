...
public class PR2OmniController : MonoBehaviour
{
    public ArticulationBody wheel0_l, wheel0_r, wheel1_l, wheel1_r, wheel2_l, wheel2_r, wheel3_l, wheel3_r;


    public float wheelRadius = 0.0762f;  
    public float robotRadius = 0.283f;   
    public float gammaDeg = 0f;          // omni wheel angle

    public float timeoutDuration = 1.0f;


    private float lastCmdTime = -Mathf.Infinity;

    const int nWheels = 4;
    float theta => 2 * Mathf.PI / nWheels;
    float gamma => Mathf.Deg2Rad * gammaDeg;
    ...
    void MoveWheels()
    {
        float vx = currentVelocity.z;
        float vy = currentVelocity.x;
        float wz = currentAngularVelocity.y;

        ArticulationBody[] wheels = { wheel0_l, wheel0_r, wheel1_l, wheel1_r, wheel2_l, wheel2_r, wheel3_l, wheel3_r};

        float[] omegas = new float[8];
        omegas[0] = (vx - vy - robotRadius * wz) / wheelRadius;
        omegas[1] = omegas[0];
        omegas[2] = (vx + vy + robotRadius * wz) / wheelRadius;
        omegas[3] = omegas[2];
        omegas[4] = (vx + vy - robotRadius * wz) / wheelRadius;
        omegas[5] = omegas[4];

        omegas[6] = (vx - vy + robotRadius * wz) / wheelRadius;
        omegas[7] = omegas[6];

        for (int i = 0; i < nWheels; i++)
        {
            ArticulationDrive drive = wheels[2*i].xDrive;
            drive.targetVelocity = omegas[2*i];
            drive.forceLimit = 7f;
            wheels[2*i].xDrive = drive;

            ArticulationDrive drive2 = wheels[2*i+1].xDrive;
            drive2.targetVelocity = omegas[2*i+1];
            drive2.forceLimit = 7f;
            wheels[2*i+1].xDrive = drive2;

            UnityEngine.Debug.LogError($"Wheel {i}: omega = {omegas[2*i]:F3}, vx = {vx:F3}, vy = {vy:F3}, wz = {wz:F3}");
        }
    }
    ...
    void StopWheels()
    {
        foreach (var w in new[] { wheel0_l, wheel0_r, wheel1_l, wheel1_r, wheel2_l, wheel2_r, wheel3_l, wheel3_r  })
            w.jointVelocity = new ArticulationReducedSpace(0f);
    }
