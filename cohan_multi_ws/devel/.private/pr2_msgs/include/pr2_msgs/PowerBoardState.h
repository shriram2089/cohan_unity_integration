...
struct Printer< ::pr2_msgs::PowerBoardState_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pr2_msgs::PowerBoardState_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "circuit_state: ";
    if (v.circuit_state.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.circuit_state.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.circuit_state[i]);
    }
    if (v.circuit_state.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "circuit_voltage: ";
    if (v.circuit_voltage.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.circuit_voltage.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.circuit_voltage[i]);
    }
    if (v.circuit_voltage.empty() || true)
      s << "]";
    if (true || !indent.empty())
