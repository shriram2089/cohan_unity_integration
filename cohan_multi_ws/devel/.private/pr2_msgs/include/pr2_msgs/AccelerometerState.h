...
struct AccelerometerState_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::geometry_msgs::Vector3_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::geometry_msgs::Vector3_<ContainerAllocator> >> _samples_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pr2_msgs::AccelerometerState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.samples.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Vector3_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.samples[i]);
    }
    if (v.samples.empty() || false)
      s << "]";
  }
