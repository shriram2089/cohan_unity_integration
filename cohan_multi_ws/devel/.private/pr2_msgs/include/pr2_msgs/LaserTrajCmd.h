...
struct LaserTrajCmd_
{
  ...
  _position_type position;
   ...
   typedef std::vector<ros::Duration, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<ros::Duration>> _time_from_start_type;
...
struct Printer< ::pr2_msgs::LaserTrajCmd_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pr2_msgs::LaserTrajCmd_<ContainerAllocator>& v)
  {
    ...
    if (v.time_from_start.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.time_from_start.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<ros::Duration>::stream(s, true ? std::string() : indent + "    ", v.time_from_start[i]);
    }
    if (v.time_from_start.empty() || true)
      s << "]";
    if (true || !indent.empty())
