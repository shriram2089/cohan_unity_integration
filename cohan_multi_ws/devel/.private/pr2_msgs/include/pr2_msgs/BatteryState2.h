...
struct Printer< ::pr2_msgs::BatteryState2_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pr2_msgs::BatteryState2_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "battery_register: ";
    if (v.battery_register.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.battery_register.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int16_t>::stream(s, true ? std::string() : indent + "    ", v.battery_register[i]);
    }
    if (v.battery_register.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "battery_update_flag: ";
    if (v.battery_update_flag.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.battery_update_flag.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.battery_update_flag[i]);
    }
    if (v.battery_update_flag.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "battery_register_update: ";
    if (v.battery_register_update.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.battery_register_update.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<ros::Time>::stream(s, true ? std::string() : indent + "    ", v.battery_register_update[i]);
    }
    if (v.battery_register_update.empty() || true)
      s << "]";
  }
