...
let AgentPath = require('./AgentPath.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class AgentPathArray {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.paths.forEach((val) => {
      bufferOffset = AgentPath.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new AgentPathArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.paths[i] = AgentPath.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.paths.forEach((val) => {
      length += AgentPath.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new AgentPathArray(null);
    ...
    if (msg.paths !== undefined) {
      ...
      for (let i = 0; i < resolved.paths.length; ++i) {
        resolved.paths[i] = AgentPath.Resolve(msg.paths[i]);
      }
    }
    else {
      resolved.paths = []
