  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hateb_local_planner::ObstacleMsg_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.obstacles.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::PolygonStamped_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.obstacles[i]);
    }
    if (v.obstacles.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "orientations: ";
    if (v.orientations.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.orientations.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::QuaternionStamped_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.orientations[i]);
    }
    if (v.orientations.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.velocities.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::TwistWithCovariance_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.velocities[i]);
    }
    if (v.velocities.empty() || false)
      s << "]";
  }
